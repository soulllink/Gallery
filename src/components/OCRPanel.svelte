<script lang="ts">
    ...
    function startDrag(e: MouseEvent, index: number) {
        if ((e.target as HTMLElement).classList.contains('close-tooltip')) return;
    ...
    function handleDrag(e: MouseEvent) {
        ...
        const result = ocrResults[dragging];
        if (!result) return;

        result.x = e.clientX - dragOffsetX;
        result.y = e.clientY - dragOffsetY;
        ocrResults = [...ocrResults]; // Trigger reactivity
    }
